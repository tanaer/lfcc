...
class UserController extends HomeController {
	...
	public function setCoinPwd(){		
		if(IS_POST){
			...
			$opwd=I('oldpassword');
			if(!$npwd=I('newpassword')) $this->error('出款密码不能为空');
	...
	public function setCBAccount(){
		if(IS_POST){
			...
			if(think_ucenter_md5(I('coinPassword'), UC_AUTH_KEY)!=$user['coinPassword']) $this->error('出款密码不正确');
	...
	public final function getTip(){
		//提现
		if($data=M('member_cash')->where(array('uid'=>$this->user['uid'], 'state'=>array('in', array('0','4')), 'isDelete'=>0, 'qflag'=>0))->order('id desc')->find()){
		...
		//充值
		if($data=M('member_recharge')->where(array('uid'=>$this->user['uid'], 'state'=>array('in', array('1','9')), 'isDelete'=>0, 'qflag'=>0))->order('id desc')->find()){
		...
		//盈亏
		if($datas=M('bets')->where(array('uid'=>$this->user['uid'], 'lotteryNo'=>array('neq', ''), 'isDelete'=>0, 'flag'=>0))->order('id desc')->select()){
	...
	public final function getTip_recharge(){
		...
		if($data=M('member_recharge')->where(array('uid'=>$this->user['uid'], 'state'=>0,'isDelete'=>0, 'actionTime'=>array('gt',strtotime(' 00:00:00'))))->field('id,flag')->select()){
