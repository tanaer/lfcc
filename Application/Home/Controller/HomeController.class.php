...
class HomeController extends Controller {
	...
	public function no0Hdq(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
		$actionNo=date('Ymd-', $time).substr(10000+$actionNo,1);
	}
	public function no0Hd_1(&$actionNo, &$actionTime, $time = null)
    {
		...
		//天津11选5
	private function no0Hdtj(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
		$actionNo=date('16md', $time).substr(1000+$actionNo,1);
	}
    ...
    //{
       // $this->setTimeNo($actionTime, $time);
		//$actionNo = intval((strtotime($actionTime) - 30 - strtotime('2016-12-20 23:57:00')) / 90) + 1720842;
		//$actionNo = strval($actionNo);
   // }
	// 韩国1.5分彩22
	private function hgssc(&$actionNo, &$actionTime, $time = null){
        $this->setTimeNo($actionTime, $time);
        $actionNo=(strtotime(date('Y-m-d 07:00:00'))-strtotime('2016-12-23 07:00:00'))/86400;
         if(date('H:i:s')<date('07:00:00')){
        $actionNo = intval(intval((strtotime($actionTime)) / 90) -14746357)-$actionNo*80;
        }elseif(date('H:i:s')>=date('07:00:00')){
        $actionNo = intval(intval((strtotime($actionTime)) / 90) -14746437)-$actionNo*80;
    }
	}
				// dj1.5分彩
	...
	//天津时时彩
		 private function no0Hd_2tj(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
		if($actionNo==0||$actionNo==120){
			//echo 999;
			$actionNo=date('Ymd-120', $time - 24*3600);
